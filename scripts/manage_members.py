...
def generate_html(title, given, middle, last, role, is_alumni, photo_path, thesis_title, thesis_link):
...
class AddMemberGUI:
    def __init__(self, root):
        ...
        self.title_combo.grid(row=0, column=1, pady=5)

        ttk.Label(frame, text="Given Name: *").grid(row=1, column=0, sticky="w", pady=5)
        self.given_entry = ttk.Entry(frame, width=30)
        self.given_entry.grid(row=1, column=1, pady=5)

        ttk.Label(frame, text="Middle Name:").grid(row=2, column=0, sticky="w", pady=5)
        self.middle_entry = ttk.Entry(frame, width=30)
        self.middle_entry.grid(row=2, column=1, pady=5)

        ttk.Label(frame, text="Last Name: *").grid(row=3, column=0, sticky="w", pady=5)
        self.last_entry = ttk.Entry(frame, width=30)
        self.last_entry.grid(row=3, column=1, pady=5)

        ttk.Label(frame, text="Role: *").grid(row=4, column=0, sticky="w", pady=5)
        self.role_combo = ttk.Combobox(frame, values=["PhD Student", "Postdoctoral Fellow", "Master Student", "Other"], width=27)
        ...
        ttk.Checkbutton(frame, text="Alumni?", variable=self.alumni_var, onvalue=True, offvalue=False).grid(row=5, column=1, sticky="w", pady=5)

        ttk.Label(frame, text="Photo:").grid(row=6, column=0, sticky="w", pady=5)
        self.photo_btn = ttk.Button(frame, text="Select Photo...", command=self.select_photo)
        self.photo_btn.grid(row=6, column=1, sticky="w", pady=5)
        self.photo_label = ttk.Label(frame, text="No photo selected", font=("Arial", 8), foreground="gray")
        self.photo_label.grid(row=7, column=1, sticky="w")

        ttk.Label(frame, text="PhD Title:").grid(row=8, column=0, sticky="w", pady=5)
        self.thesis_title_entry = ttk.Entry(frame, width=30)
        self.thesis_title_entry.grid(row=8, column=1, pady=5)

        ttk.Label(frame, text="Thesis Link:").grid(row=9, column=0, sticky="w", pady=5)
        self.thesis_link_entry = ttk.Entry(frame, width=30)
        self.thesis_link_entry.grid(row=9, column=1, pady=5)

        ttk.Label(frame, text="PhD Completed?").grid(row=10, column=0, sticky="w", pady=5)
        self.phd_completed_var = tk.BooleanVar(self.root)
        self.phd_completed_var.set(False)
        ttk.Checkbutton(frame, text="Yes", variable=self.phd_completed_var, onvalue=True, offvalue=False).grid(row=10, column=1, sticky="w", pady=5)

        ttk.Button(frame, text="Add Member", command=self.add_member).grid(row=11, column=0, columnspan=2, pady=20)
    ...
    def add_member(self):
        ...
        try:
            ...
            member_html = generate_html(
                title, given, middle, last, role, is_alumni, photo_dest_web_path, thesis_title, thesis_link
            )
            ...
            # Update theses.html if a thesis title is provided
            if thesis_title:
                name = format_name(given, middle, last)
                insert_thesis_into_html(THESES_FILE, name, thesis_title, thesis_link, is_phd_completed)

            messagebox.showinfo("Success", f"Member {given} {last} added successfully!")
            self.clear_form()
...
if __name__ == "__main__":
    if not os.path.exists(HTML_FILE):
        print(f"Error: {HTML_FILE} not found. Please run from the scripts folder or ensure group.html exists in the parent directory.")
    else:
        root = tk.Tk()
